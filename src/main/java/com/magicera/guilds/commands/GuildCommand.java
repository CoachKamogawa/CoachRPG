...
public final class Guild {
    ...
    public void setFoundedAtEpochMs(long foundedAtEpochMs) { this.foundedAtEpochMs = Math.max(0L, foundedAtEpochMs); }
    ...
    public void setTaxPercent(int taxPercent) { this.taxPercent = Math.max(0, Math.min(5, taxPercent)); }
    ...
    public boolean isAlliedWith(String otherGuildId) {
        return otherGuildId != null && allies.contains(otherGuildId);
    }

    public boolean isEnemyWith(String otherGuildId) {
        return otherGuildId != null && enemies.contains(otherGuildId);
    }

    public void addClaimedChunk(String key) {
        if (key != null && !key.isEmpty()) claimedChunks.add(key);
    }

    public void removeClaimedChunk(String key) {
        if (key != null) claimedChunks.remove(key);
    }

    public void addAlly(String guildId) {
        if (guildId != null && !guildId.isEmpty()) allies.add(guildId);
    }

    public void removeAlly(String guildId) {
        if (guildId != null) allies.remove(guildId);
    }

    public void addEnemy(String guildId) {
        if (guildId != null && !guildId.isEmpty()) enemies.add(guildId);
    }

    public void removeEnemy(String guildId) {
        if (guildId != null) enemies.remove(guildId);
    }
}
